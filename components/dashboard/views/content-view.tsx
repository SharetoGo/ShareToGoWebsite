...
export function ContentView() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      if (fetchedPosts.length > 0) {
        setActiveTab((prev) => prev || fetchedPosts[0].id);
      }
    });
  }, [companyData?.id]);
  ...
  const activeContent = useMemo(() => {
    return (
      [...suggestedItems, ...libraryPosts].find((i) => i.id === activeTab) || suggestedItems[0]
    );
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-10">
      ...
      <div className="bg-[#2a2c38] p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10">
          <StatMiniWhite
            label="CO2e EVITADO"
            value={`${formatStats(stats.co2)} kg`}
            sub={`${stats.trees} Árboles`}
          />
              ...
              {libraryPosts.map((post) => (
                <div key={post.id} className="group relative">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      deleteDoc(doc(db, "companies", companyData.id, "contentPosts", post.id));
                    }}
          ...
          {isCreating ? (
            <div className="bg-white rounded-[3rem] border border-gray-100 shadow-2xl p-10 space-y-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setIsCreating(false)} className="p-2 bg-gray-50 rounded-xl">
                  <X />
                </button>
              </div>
              <input
...
function TabButton({ active, onClick, icon, label, disabled }: any) {
  return (
    ...
    >
      ...
      {disabled && !active && (
        <span className="text-[8px] font-bold text-gray-300 uppercase">Vacío</span>
      )}
    </button>
  );
}
