...
export function EmployeesView() {
  ...
  const handleDelete = async (empId: string) => {
    if (!companyData?.id || !confirm("¿Estás seguro de que deseas eliminar a este empleado de la empresa?")) return;
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium italic">Gestiona el impacto de tus colaboradores.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {employees.slice(0, 3).map((emp, i) => (
          <div key={emp.id} className="bg-white p-6 rounded-4xl border border-gray-100 shadow-sm flex items-center gap-5 relative overflow-hidden group hover:border-[#9dd187] transition-all">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gray-100 shrink-0 flex items-center justify-center text-[10px] font-bold text-gray-400 overflow-hidden uppercase">
                             {emp.profilePicture ? <Image src={emp.profilePicture} alt="" width={40} height={40} className="object-cover"/> : emp.name[0]}
                        </div>
                        <div>
                            <p className="font-bold text-[#2a2c38]">{emp.name} {emp.lastName}</p>
        ...
        <div className="fixed inset-0 bg-[#2a2c38]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                    ...
                    <button onClick={() => setEditingEmployee(null)} className="text-gray-300 hover:text-red-500 transition-colors"><X size={24}/></button>
                </div>
