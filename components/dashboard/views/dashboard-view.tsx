...
function getMonthIds() {
  ...
  const previousMonth = `${prevDate.getFullYear()}-${String(prevDate.getMonth() + 1).padStart(
    2,
    "0",
  )}`;
...
export function DashboardView({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  ...
  const co2Progress = co2Target && co2Target > 0 ? Math.min((totalCo2 / co2Target) * 100, 100) : 0;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_1.6fr] gap-6">
        ...
        <AnalyticsHub totalCo2={totalCo2} onViewAnalytics={() => setActiveTab("analytics")} />
        <TopCarpoolers />
      </div>
    </div>
  );
