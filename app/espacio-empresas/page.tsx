...
const SolutionCard = ({ icon: Icon, badge, title, text, items, primaryIcon = false, t }: any) => (
  <Card className="p-8 border-2 hover:border-[#9dd187] transition-colors flex flex-col h-full">
    <CardContent className="pt-6 flex-1 flex flex-col p-0">
      <div
        className={`w-16 h-16 rounded-xl flex items-center justify-center mb-6 ${primaryIcon ? "bg-[#2a2c38]" : "bg-[#9dd187]"}`}
      >
      ...
      <Link href="/contratar" className="mt-auto">
        <Button
          className={`w-full ${primaryIcon ? "bg-[#2a2c38] hover:bg-[#1a1c24]" : "bg-[#9dd187] hover:bg-[#8bc475]"} text-white`}
        >
          {t("ee_sol_more")}
        </Button>
      </Link>
  ...
  <Card className="border-gray-100 shadow-xl shadow-gray-200/50 rounded-[2.5rem] overflow-hidden hover:shadow-2xl transition-all duration-300">
    <CardContent className="p-10">
      <div className="flex items-center gap-4 mb-6">
        <div
          className={`w-14 h-14 rounded-2xl flex items-center justify-center ${darkIcon ? "bg-[#1a1c24]" : "bg-[#9dd187]/10"}`}
        >
      ...
      <blockquote className="text-gray-600 mb-8 italic text-lg leading-relaxed">{quote}</blockquote>
      <div className="grid grid-cols-3 gap-4 text-center bg-gray-50 rounded-2xl p-6">
        {metrics.map((m: any, i: number) => (
          <div key={i} className="border-r border-gray-200 last:border-0">
            ...
            <div className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-1">
              {m.label}
            </div>
          </div>
        ))}
            ...
            <div className="text-white">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button
                  ...
                  onClick={() =>
                    document.getElementById("casos-exito")?.scrollIntoView({ behavior: "smooth" })
                  }
                >
                  {t("ee_hero_cta_cases")}
                </Button>
